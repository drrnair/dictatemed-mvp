...
export async function queueStyleAnalysis(
  ...
  try {
    await runStyleAnalysis({ userId, subspecialty });
...
function parseSubspecialtyAnalysisResponse(
  ...
  try {
    ...
    const result: SubspecialtyStyleAnalysisResult = {
      ...
      detectedSectionVerbosity: (parsed.detectedSectionVerbosity ?? {}) as SectionVerbosityMap,
...
export function mergeProfileAnalysis(
  ...
  const mergedSectionOrder =
    analysis.confidence.sectionOrder > (existing.confidence.sectionOrder ?? 0)
  ...
  const mergedSectionVerbosity =
    analysis.confidence.sectionVerbosity > (existing.confidence.sectionVerbosity ?? 0)
  ...
  const mergedTerminologyLevel =
    analysis.confidence.terminologyLevel > (existing.confidence.terminologyLevel ?? 0)
      ? analysis.detectedTerminologyLevel
      : existing.terminologyLevel;

  const mergedGreetingStyle =
    analysis.confidence.greetingStyle > (existing.confidence.greetingStyle ?? 0)
      ? analysis.detectedGreetingStyle
      : existing.greetingStyle;

  const mergedClosingStyle =
    analysis.confidence.closingStyle > (existing.confidence.closingStyle ?? 0)
      ? analysis.detectedClosingStyle
      : existing.closingStyle;

  const mergedSignoff =
    analysis.confidence.signoffTemplate > (existing.confidence.signoffTemplate ?? 0)
      ? analysis.detectedSignoff
      : existing.signoffTemplate;

  const mergedFormalityLevel =
    analysis.confidence.formalityLevel > (existing.confidence.formalityLevel ?? 0)
      ? analysis.detectedFormalityLevel
      : existing.formalityLevel;

  const mergedParagraphStructure =
    analysis.confidence.paragraphStructure > (existing.confidence.paragraphStructure ?? 0)
