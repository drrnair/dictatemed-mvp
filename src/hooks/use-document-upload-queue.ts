    ...
    (newFiles: File[]): FileValidationError[] => {
      ...
      if (newFiles.length > availableSlots) {
        ...
        errors.push({
          ...
          error: `Maximum ${MAX_BATCH_FILES} files allowed. ${newFiles.length} files added.`,
        });
      }
    ...
    try {
      ...
      while (uploadQueue.length > 0 || activeUploads.length > 0) {
        ...
        while (uploadQueue.length > 0 && activeUploads.length < MAX_CONCURRENT_UPLOADS) {
          const queuedFile = uploadQueue.shift()!;
          ...
          const uploadPromise = processFile(
            queuedFile,
            uploadInfo,
        ...
        if (activeUploads.length > 0) {
          ...
          // Remove completed promises (they resolve to undefined)
          const remainingUploads: Promise<void>[] = [];
          for (let i = 0; i < activeUploads.length; i++) {
            const result = await Promise.race([
              activeUploads[i].then(() => 'resolved'),
              Promise.resolve('pending'),
            ]);
            if (result === 'pending') {
              remainingUploads.push(activeUploads[i]);
            }
          }
          activeUploads.length = 0;
          activeUploads.push(...remainingUploads);
        }
      }
    } finally {
      setIsProcessing(false);
