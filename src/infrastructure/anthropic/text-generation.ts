// src/infrastructure/anthropic/text-generation.ts
// Claude text generation client for letter generation via direct Anthropic API

import { getAnthropicClient } from './client';
import { logger } from '@/lib/logger';

// Model IDs for direct Anthropic API (without 'anthropic.' prefix)
export const MODELS = {
 ...
 * Handles transient errors like rate limiting.
 */
export async function generateTextWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Don't retry on non-retryable errors (400 Bad Request, 401 Unauthorized, etc.)
...
export function estimateCost(
  ...
  // Pricing per 1M tokens (USD) - Direct Anthropic API pricing
